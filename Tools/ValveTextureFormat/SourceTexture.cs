...
namespace Chisel.Import.Source.VPKTools
{
    public class SourceTexture
    {
        ...
        {
            SourceTexture vtf;
            using( MemoryStream ms = new MemoryStream( vtfData ) ) { vtf = ReadAndCache( ms, 0, location ); }
        ...
        public static SourceTexture ReadAndCache( Stream stream, long origOffset, string location )
        {
            ...
            if( loadedTextures.ContainsKey( fixedLocation ) ) { srcTexture = loadedTextures[fixedLocation]; }
            else
            {
        ...
        public static SourceTexture GrabTexture( VPKParser vpkParser, string rawPath )
        {
            SourceTexture srcTexture = null;

            if( !string.IsNullOrEmpty( rawPath ) )
            {
                string vtfFilePath = FixLocation( vpkParser, rawPath );
                if( !string.IsNullOrEmpty( vtfFilePath ) )
                {
                    if( loadedTextures.ContainsKey( vtfFilePath ) ) { srcTexture = loadedTextures[vtfFilePath]; }
                    else
                    {
                        if( vpkParser != null && vpkParser.FileExists( vtfFilePath ) )
                        {
                            try { vpkParser.LoadFileAsStream( vtfFilePath, ( stream, origOffset, fileLength ) => { srcTexture = ReadAndCache( stream, origOffset, vtfFilePath ); } ); }
                            catch( Exception e ) { Debug.LogError( "SourceTexture: " + e.ToString() ); }
                        }
                        else
        ...
        public static string FixLocation( VPKParser vpkParser, string rawPath )
        {
            ...
            if( !Path.GetExtension( fixedLocation ).Equals( ".vtf" ) && ( vpkParser == null || !vpkParser.FileExists( fixedLocation ) ) )
                fixedLocation += ".vtf";
